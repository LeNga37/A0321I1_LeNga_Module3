...
select dvdk.*, count(hdct.id_dich_vu_di_kem) as so_lan_su_dung
...
inner join dich_vu dv on dv.id_loai_dich_vu = ldv.id_loai_dich_vu
...
-- Task 15.	Hiển thi thông tin của tất cả nhân viên bao gồm IDNhanVien, HoTen, TrinhDo, TenBoPhan, SoDienThoai, DiaChi mới chỉ lập được tối đa 3 hợp đồng từ năm 2018 đến 2021.
select nv.id_nhan_vien, nv.ho_ten, td.trinh_do, bp.ten_bo_phan, nv.sdt, nv.dia_chi, count(hd.id_nhan_vien) as so_lan_lap_hop_dong, year(hd.ngay_lam_hop_dong) as nam_lam_hop_dong
from nhan_vien nv
